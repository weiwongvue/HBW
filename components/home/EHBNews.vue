<template>
  <div class="eHBNews">
    <div class="container HBNews">
      <div class="row">
        <div class="col-12">
          <h2>华邦动态</h2>
        </div>
        <div class="col-12 HBNewsNav">
          <span>当前位置:首页>华邦动态>学历教育</span>
        </div>
        <div class="col-2 HBNewsOptions">
          <h3>COURSE</h3>
          <ul>
            <li><a href="javascript:void(0);" class="HBNewsOptionSelect">学历教育</a></li>
            <li><a href="javascript:void(0);">学历教育</a></li>
            <li><a href="javascript:void(0);">学历教育</a></li>
            <li><a href="javascript:void(0);">学历教育</a></li>
            <li><a href="javascript:void(0);">学历教育</a></li>
            <li><a href="javascript:void(0);">学历教育</a></li>
          </ul>
        </div>
        <div class="col-10 HBNewsContent">
          <div class="HBNewsContentBorder"></div>
          <div class="HBNewsContentImg">
            <div class="HBNewsContentImgLeft">
              <p>深圳成人学历教育指定报考中心</p>
            </div>
            <div class="HBNewsContentImgRight">
              <div class="HBNewsContentImgRight1">
                <p>深圳成人学历教育指定报考中心</p>
              </div>
              <div class="HBNewsContentImgRight2">
                <p>深圳成人学历教育指定报考中心</p>
              </div>
            </div>
          </div>
          <ul class="HBNewsList">
            <li class="HBNewsListLi">
              <div class="HBNewsList1">
                <img src="~static/home/hbimg4.png" alt="华邦" title="华邦">
              </div>
              <div class="HBNewsList2">
                <h5>标题标题标题标题标题标题标题标题</h5>
                <p>内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容.......</p>
                <span>发布时间：2020-11-6</span>
              </div>
            </li>
            <li class="HBNewsListLi">
              <div class="HBNewsList1">
                <img src="~static/home/hbimg4.png" alt="华邦" title="华邦">
              </div>
              <div class="HBNewsList2">
                <h5>标题标题标题标题标题标题标题标题</h5>
                <p>内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容.......</p>
                <span>发布时间：2020-11-6</span>
              </div>
            </li>
            <li class="HBNewsListLi">
              <div class="HBNewsList1">
                <img src="~static/home/hbimg4.png" alt="华邦" title="华邦">
              </div>
              <div class="HBNewsList2">
                <h5>标题标题标题标题标题标题标题标题</h5>
                <p>内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容.......</p>
                <span>发布时间：2020-11-6</span>
              </div>
            </li>
            <li class="HBNewsListLi">
              <div class="HBNewsList1">
                <img src="~static/home/hbimg4.png" alt="华邦" title="华邦">
              </div>
              <div class="HBNewsList2">
                <h5>标题标题标题标题标题标题标题标题</h5>
                <p>内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容.......</p>
                <span>发布时间：2020-11-6</span>
              </div>
            </li>
            <li class="HBNewsListLi">
              <div class="HBNewsList1">
                <img src="~static/home/hbimg4.png" alt="华邦" title="华邦">
              </div>
              <div class="HBNewsList2">
                <h5>标题标题标题标题标题标题标题标题</h5>
                <p>内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容.......</p>
                <span>发布时间：2020-11-6</span>
              </div>
            </li>
            <li class="HBNewsListLi">
              <div class="HBNewsList1">
                <img src="~static/home/hbimg4.png" alt="华邦" title="华邦">
              </div>
              <div class="HBNewsList2">
                <h5>标题标题标题标题标题标题标题标题</h5>
                <p>内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容内容.......</p>
                <span>发布时间：2020-11-6</span>
              </div>
            </li>
          </ul>
          <div class="HBNewsListIndex">
            <a href="javascript:void(0);">1</a>
            <a href="javascript:void(0);">2</a>
            <a href="javascript:void(0);">3</a>
            <a href="javascript:void(0);">下一页</a>
            <span>共3页</span>
            <span>到第</span>
            <input type="text" class="page" name="" id="" value="" />
            <span>页</span>
            <button type="button">确定</button>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped>
/** {*/
/*  margin: 0 auto;*/
/*  padding: 0;*/
/*}*/

a {
  color: #000000;
}

li {
  list-style: none;
}


.HBNews h2 {
  font-weight: 600;
  text-align: center;
  margin-top: 30px;
}

.HBNews .HBNewsNav {
  text-align: right;
  color: #777777;
}

.HBNewsOptions h3 {
  text-align: center;
  font-size: 35px;
}

.HBNewsOptions li a {
  display: block;
  width: 100%;
  height: 40px;
  line-height: 40px;
  text-align: center;
  border-bottom: 1px solid #DFDFDF;
  font-size: 18px;
}

.HBNewsOptions li .HBNewsOptionSelect {
  border: 1px solid #252933;
}

.HBNewsContentBorder {
  height: 5px;
  width: 100%;
  background-color: #314A7F;
  margin-top: 10px;
}

.HBNewsContentImg {
  margin-top: 30px;
  height: 350px;
}

.HBNewsContentImgLeft {
  width: 70%;
  height: 350px;
  background-image: url("~static/home/hbimg.png");
  background-size: cover;
  background-repeat: no-repeat;
  background-position: top center;
  float: left;
  position: relative;
}

.HBNewsContentImg p {
  height: 35px;
  width: 100%;
  line-height: 35px;
  position: absolute;
  bottom: 0;
  color: #FFFFFF;
  background-color: rgba(52, 52, 54, .5);
  margin-bottom: 0;
  padding-left: 10px;
}

.HBNewsContentImgRight {
  margin-left: 1%;
  height: 350px;
  width: 29%;
  float: left;
}

.HBNewsContentImgRight p {
  height: 25px;
  line-height: 25px;
  font-size: 12px;
}

.HBNewsContentImgRight1 {
  height: 48.5%;
  background-image: url("~static/home/hbimg2.png");
  background-size: cover;
  background-repeat: no-repeat;
  background-position: top center;
  position: relative;
}

.HBNewsContentImgRight2 {
  margin-top: 4%;
  height: 48.5%;
  background-image: url("~static/home/hbimg3.png");
  background-size: cover;
  background-repeat: no-repeat;
  background-position: top center;
  position: relative;
}

.HBNewsList {}

.HBNewsListLi {
  margin-top: 30px;
  width: 100%;
  height: 190px;
  border-bottom: 1px solid #DFDFDF;
}

.HBNewsList1 {
  float: left;
  width: 30%;
  height: 160px;
}

.HBNewsList1 img {
  width: 100%;
  height: 100%;
}

.HBNewsList2 {
  float: left;
  width: 70%;
  height: 160px;
  position: relative;
  padding: 5px 10px 0;
}

.HBNewsList2 p {
  font-size: 14px;
  height: 84px;
  overflow: hidden;
}

.HBNewsList2 span {
  font-size: 14px;
  position: absolute;
  bottom: 0;
}

.HBNewsListIndex {
  text-align: center;
}

.HBNewsListIndex a {
  display: inline-block;
  height: 35px;
  line-height: 25px;
  padding: 5px 15px;
  border: 1px solid #DFDFDF;
}

.HBNewsListIndex * {
  margin-right: 10px;
  color: #010000;
  font-size: 14px;
}

.HBNewsListIndex .page {
  height: 35px;
  width: 39px;
  border: 1px solid #DFDFDF;
}

.HBNewsListIndex button {
  height: 35px;
  padding: 5px 15px;
  background-color: #FFFFFF;
  border: 1px solid #DFDFDF;
}

</style>
